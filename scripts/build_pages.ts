...
async function bundle(entrypoint: string, outfile: string) {
  const result = await Bun.build({
    define: {
      __ZIP_CAT_STATIC_BUILD__: "false"
    },
...
async function assertNoSecretValues(paths: string[]) {
  ...
  for (const path of paths) {
    ...
    const found = secretValues.find((value) => text.includes(value));
    if (found) {
      throw new Error(`Pages artifact ${path} includes a secret value.`);
    }
  }
}
...
console.log(`Cloudflare Pages zip.cat written to ${outDir}`);
